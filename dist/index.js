...
const DEFAULT_ENDIAN = process.env.ENDIAN === "be" ? "be" : "le";
...
const fromHex = (h) => Buffer.from(h.replace(/^0x/i, ""), "hex");
const isPrint = (b) => b.length && [...b].every((v) => v >= 0x20 && v < 0x7f);
const toAscii = (b) => b.toString("utf8");
const pretty = (b) => [...b]
    .map((c) => c >= 0x20 && c < 0x7f
...
function parseAlkaneId(str) {
    const m = str.match(/^(\d+):(\d+)$/);
    if (!m)
        throw new Error("AlkaneId must be A:B (decimal u128)");
    return [BigInt(m[1]), BigInt(m[2])];
}
function makePrefix(id, endian) {
...
function normalizeSuffix(ascii) {
...
function extractContractKeyHex(kbuf) {
    const marker = Buffer.from("/storage");
    const idx = kbuf.indexOf(marker);
    if (idx === -1)
        return toHex(kbuf); // fallback
    ...
    if (lastSlash !== -1 && lastSlash < tail.length - 1) {
        ...
        if (digitsOnly)
            tail = tail.slice(0, lastSlash);
    }
    return toHex(tail);
}
function decodeVal(buf) {
    if (isPrint(buf)) {
        const text = toAscii(buf);
        const m = text.match(/^(\d+):([0-9a-fA-F]+)0*$/);
        if (m)
            return { text, block: Number(m[1]), hex: m[2].toLowerCase() };
        return { text };
    }
...
const parseLdb = (out) => out.split(/\r?\n/).flatMap((l) => {
...
app.get("/dump/:alkaneId", async (req, res) => {
    ...
    try {
        const endian = req.query.endian === "be" ? "be" : DEFAULT_ENDIAN;
